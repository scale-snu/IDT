...
NTHREADS=4 #roms has already 8 process!
...
if [[ "${BENCH_SIZE}" == "small" ]]; then
	BENCH_RUN+="${BENCH_BIN} 654 0 4"
elif [[ "${BENCH_SIZE}" == "large" ]]; then
	BENCH_RUN+="${BENCH_BIN} 654 0 8"
else
	err "ERROR: Retry with available SIZE. refer to benches/_roms.sh"
	exit -1
fi

export BENCH_RUN
export BENCH_NAME="sroms_base.gcc-baseline-o2-m64"

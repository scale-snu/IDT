      ...
      switch (op)
      {
        ...
        case DivideComplexOperator:
        {
...
#if defined(SPEC)
          gamma=(Br->red*Br->red+Bi->red*Bi->red+snr);
...
#if defined(SPEC)
          gamma=(Br->green*Br->green+Bi->green*Bi->green+snr);
          if (gamma < 0.0) gamma = 0.0;
          gamma=PerceptibleReciprocal(gamma);
#else
...
#if defined(SPEC)
          gamma=(Br->blue*Br->blue+Bi->blue*Bi->blue+snr);
          if (gamma < 0.0) gamma = 0.0;
          gamma=PerceptibleReciprocal(gamma);
#else
          ...
          if (images->matte != MagickFalse)
            {
#if defined(SPEC)
              gamma=(Br->opacity*Br->opacity+Bi->opacity*
                Bi->opacity+snr);
              if (gamma < 0.0) gamma = 0.0;
              gamma=PerceptibleReciprocal(gamma);
#else
  ...
  if ((value == (const char *) NULL) || (LocaleCompare(value,"forward") == 0))
    {
      ...
      */
      i=0L;
#if defined(SPEC)
      gamma=((double) fourier_info->width*
          fourier_info->height);
      if (gamma < 0.0) gamma = 0.0;
      gamma=PerceptibleReciprocal(gamma);
#else

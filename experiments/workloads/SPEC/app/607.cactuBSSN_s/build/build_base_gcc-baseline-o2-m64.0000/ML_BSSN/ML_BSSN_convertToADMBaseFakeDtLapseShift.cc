...
static void ML_BSSN_convertToADMBaseFakeDtLapseShift_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
...
#endif
  CCTK_LOOP3(ML_BSSN_convertToADMBaseFakeDtLapseShift,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    {
      dtbetaxL = theta*(Xt1L + beta1L*eta*ToReal(BetaDriver)*(-1 + 
        ToReal(ShiftBCoeff)) + (B1L - 
        Xt1L)*ToReal(ShiftBCoeff))*ToReal(ShiftGammaCoeff);
      
      dtbetayL = theta*(Xt2L + beta2L*eta*ToReal(BetaDriver)*(-1 + 
        ToReal(ShiftBCoeff)) + (B2L - 
        Xt2L)*ToReal(ShiftBCoeff))*ToReal(ShiftGammaCoeff);
      
      dtbetazL = theta*(Xt3L + beta3L*eta*ToReal(BetaDriver)*(-1 + 
  ...
  CCTK_ENDLOOP3(ML_BSSN_convertToADMBaseFakeDtLapseShift);
}
...
extern "C" void ML_BSSN_convertToADMBaseFakeDtLapseShift(CCTK_ARGUMENTS)
{
  ...
  GenericFD_AssertGroupStorage(cctkGH, "ML_BSSN_convertToADMBaseFakeDtLapseShift", 9, groups);

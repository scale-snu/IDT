/*@@
   ...
   @desc
            Creates the CCTK variables for thorn CoordGauge
   @enddesc
 @@*/
...
#endif /* END SPEC */ 
#ifdef SPEC 
#include "CoordGauge/cctk_Arguments.h" 
#else 
#include "CoordGauge/cctk_Arguments.h"
#endif /* END SPEC */ 
...
static int CCTKi_BindingsFortranWrapperCoordGauge(void *_GH, void *fpointer);

static int CCTKi_BindingsFortranWrapperCoordGauge(void *_GH, void *fpointer)
{
  cGH const *const GH = _GH;
  const int _cctk_zero = 0;
  void (*function)(COORDGAUGE_C2F_PROTO);
  DECLARE_COORDGAUGE_C2F
  INITIALISE_COORDGAUGE_C2F
  (void) (_cctk_zero + 0);

  function = (void (*) (COORDGAUGE_C2F_PROTO)) fpointer;
  function (PASS_COORDGAUGE_C2F (GH));
...
int CactusBindingsVariables_CoordGauge_Initialise(void)
{
  ...
  if (CCTKi_CreateGroup ("SLICING_FLAGS", "COORDGAUGE", "COORDGAUGE",
                         "SCALAR", "INT", "PUBLIC",
                         0, 1,
                         "CONSTANT",
                         "", "",
  ...
  {
    warn_mixeddim_gfs = "slicing_flags";
    warn_mixeddim = 0;
  }
  ...
  CCTKi_RegisterFortranWrapper("CoordGauge", CCTKi_BindingsFortranWrapperCoordGauge);

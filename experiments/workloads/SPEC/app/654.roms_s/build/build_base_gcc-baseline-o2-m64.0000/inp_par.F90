...
!
      sequence=759
#ifndef SPEC
! random number generator is not used in this SPEC CPU benchmark
...
#else
                  Npts=load_i1(Nval, Rval, Nmodels);
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Lreport);
#endif
...
#else
                    Npts=load_i1(Nval, Rval, Nthreads(Iocean));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nthreads(Iwaves));
#endif
            END IF
          ELSE IF (TRIM(KeyWord).eq.'Nthreads(waves)') THEN
...
#else
                    Npts=load_i1(Nval, Rval, Nthreads(Iatmos));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nexport(Iocean));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nexport(Iwaves));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nexport(Iatmos));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nimport(Iocean));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nimport(Iwaves));
#endif
            END IF
...
#else
                    Npts=load_i1(Nval, Rval, Nimport(Iatmos));
#endif
            END IF
...
#else
                  Npts=load_r1(Nval, Rval, Ritz_tol);
#endif
#endif
...
#  ifdef ANA_BIOLOGY
     ...
     &            'Small detritus initial concentration (mmol/m3).'
#   ifdef IRON_LIMIT
            WRITE (out,100) BioIni(iFphy,ng), 'BioIni(iFphy)',          &
     &            'Phyto-assoc. Fe initial concentration (mumol/m3).'
            WRITE (out,100) BioIni(iFdis,ng), 'BioIni(iFdis)',          &
...
!
  ...
  30  FORMAT (/,' read_BioPar - Error while processing line: ',/,a)
  40  FORMAT (/,/,' NPZD-IRON Model Parameters, Grid: ',i2.2,           &
     &        /,  ' ====================================',/)
  50  FORMAT (1x,i10,2x,a,t30,a)
  60  FORMAT (10x,l1,2x,a,t30,a,i2.2,':',1x,a)
  90  FORMAT (1p,e11.4,2x,a,'(',i2.2,')',t30,a,/,t32,a,i2.2,':',1x,a)
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(2,isFsur));
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(2,isUbar));
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(2,isVbar));
#endif
#  ifdef SOLVE3D
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(2,isUvel));
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(2,isVvel));
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(1,isUstr));
#endif
...
#else
                  Npts=load_l1(Nval, Cval, Cnorm(1,isVstr));
#endif
#  endif
#  if defined ADJUST_STFLUX && defined SOLVE3D
...
#else
                  Npts=load_i1(Nval, Rval, Nrandom);
#endif
      ...
      DO WHILE (.TRUE.)
        ...
        IF (status.gt.0) THEN
          ...
          ELSE IF (TRIM(KeyWord).eq.'Sout(idEmPf)') THEN
            Npts=load_l(Nval, Cval, Ngrids, Sout(idEmPf,1))

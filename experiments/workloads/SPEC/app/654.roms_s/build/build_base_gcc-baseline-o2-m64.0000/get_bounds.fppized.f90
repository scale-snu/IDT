...
!
      IF (Istr.eq.1) THEN      ! Western Boundary Tile
        IstrR=Istr
        IstrT=IstrR
        IstrU=Istr
      ELSE                        ! Interior Tile
        ...
        IstrU=Istr
      END IF
      IF (Iend.eq.Lm(ng)) THEN      ! Eastern Boundary Tile
        IendR=Iend
        IendT=IendR
      ELSE                        ! Interior Tile
        IendR=Iend
        IendT=IendR
      END IF
      IF (Jstr.eq.1) THEN     ! Southern Boundary Tile
        JstrR=Jstr-1
        JstrT=JstrR
        JstrV=Jstr+1
      ELSE                        ! Interior Tile
        ...
        JstrV=Jstr
      END IF
      IF (Jend.eq.Mm(ng)) THEN     ! Northern Boundary Tile
        JendR=Jend+1
        JendT=JendR
      ELSE                        ! Interior Tile
        JendR=Jend

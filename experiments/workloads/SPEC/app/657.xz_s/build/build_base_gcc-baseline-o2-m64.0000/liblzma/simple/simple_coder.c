...
static lzma_ret
copy_or_code(lzma_coder *pcoder, lzma_allocator *allocator,
...
static size_t
call_filter(lzma_coder *pcoder, uint8_t *buffer, size_t size)
{
	struct lzma_coder_s* coder = pcoder;
...
static lzma_ret
simple_code(lzma_coder *pcoder, lzma_allocator *allocator,
		const uint8_t *restrict in, size_t *restrict in_pos,
		size_t in_size, uint8_t *restrict out,
		size_t *restrict out_pos, size_t out_size, lzma_action action)
{
...
static void
simple_coder_end(lzma_coder *pcoder, lzma_allocator *allocator)
{
	struct lzma_coder_s* coder = pcoder;
...
static lzma_ret
simple_coder_update(lzma_coder *pcoder, lzma_allocator *allocator,
		...
		uint32_t alignment, bool is_encoder)
{
	...
	if (next->coder == NULL) {
		...
		((struct lzma_coder_s*)next->coder)->filter = filter;
		((struct lzma_coder_s*)next->coder)->allocated = 2 * unfiltered_max;

		// Allocate memory for filter-specific data structure.
		if (simple_size > 0) {
			((struct lzma_coder_s*)next->coder)->simple = lzma_alloc(
					simple_size, allocator);
			if (((struct lzma_coder_s*)next->coder)->simple == NULL)
				return LZMA_MEM_ERROR;
		} else {
			((struct lzma_coder_s*)next->coder)->simple = NULL;
		}
	}

	if (filters[0].options != NULL) {
		const lzma_options_bcj *simple = filters[0].options;
		((struct lzma_coder_s*)next->coder)->now_pos = simple->start_offset;
		if (((struct lzma_coder_s*)next->coder)->now_pos & (alignment - 1))
			return LZMA_OPTIONS_ERROR;
	} else {
		((struct lzma_coder_s*)next->coder)->now_pos = 0;
	}

	// Reset variables.
	((struct lzma_coder_s*)next->coder)->is_encoder = is_encoder;
	((struct lzma_coder_s*)next->coder)->end_was_reached = false;
	((struct lzma_coder_s*)next->coder)->pos = 0;
	((struct lzma_coder_s*)next->coder)->filtered = 0;
	((struct lzma_coder_s*)next->coder)->size = 0;

	return lzma_next_filter_init(
			&((struct lzma_coder_s*)next->coder)->next, allocator, filters + 1);
}

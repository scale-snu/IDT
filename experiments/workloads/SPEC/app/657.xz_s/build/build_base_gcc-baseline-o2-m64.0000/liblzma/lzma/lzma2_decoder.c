...
static lzma_ret
lzma2_decode(lzma_coder */*restrict*/ pcoder, lzma_dict *restrict dict,
...
static void
lzma2_decoder_end(lzma_coder *pcoder, lzma_allocator *allocator)
{
		...
		const void *opt, lzma_lz_options *lz_options)
{
	if (lz->coder == NULL) {
		...
		((struct lzma_coder_s*)lz->coder)->lzma = LZMA_LZ_DECODER_INIT;
	}

	const lzma_options_lzma *options = opt;

	((struct lzma_coder_s*)lz->coder)->sequence = SEQ_CONTROL;
	((struct lzma_coder_s*)lz->coder)->need_properties = true;
	((struct lzma_coder_s*)lz->coder)->need_dictionary_reset = options->preset_dict == NULL
			|| options->preset_dict_size == 0;

	return lzma_lzma_decoder_create(&((struct lzma_coder_s*)lz->coder)->lzma,

...
///
static lzma_ret
fill_window(lzma_coder *pcoder, lzma_allocator *allocator, const uint8_t *in,
...
static lzma_ret
lz_encode(lzma_coder *pcoder, lzma_allocator *allocator,
...
static void
lz_encoder_end(lzma_coder *pcoder, lzma_allocator *allocator)
{
	struct lzma_coder_s* coder = pcoder;
...
static lzma_ret
lz_encoder_update(lzma_coder *pcoder, lzma_allocator *allocator,
			...
			lzma_lz_options *lz_options))
{
	...
	if (next->coder == NULL) {
		...
		((struct lzma_coder_s*)next->coder)->lz.coder = NULL;
		((struct lzma_coder_s*)next->coder)->lz.code = NULL;
		((struct lzma_coder_s*)next->coder)->lz.end = NULL;

		((struct lzma_coder_s*)next->coder)->mf.buffer = NULL;
		((struct lzma_coder_s*)next->coder)->mf.hash = NULL;
		((struct lzma_coder_s*)next->coder)->mf.hash_size_sum = 0;
		((struct lzma_coder_s*)next->coder)->mf.sons_count = 0;
	...
	lzma_lz_options lz_options;
	return_if_error(lz_init(&((struct lzma_coder_s*)next->coder)->lz, allocator,
			filters[0].options, &lz_options));

	// Setup the size information into next->coder->mf and deallocate
	// old buffers if they have wrong size.
	if (lz_encoder_prepare(&((struct lzma_coder_s*)next->coder)->mf, allocator, &lz_options))
		return LZMA_OPTIONS_ERROR;

	// Allocate new buffers if needed, and do the rest of
	// the initialization.
	if (lz_encoder_init(&((struct lzma_coder_s*)next->coder)->mf, allocator, &lz_options))
		return LZMA_MEM_ERROR;

	// Initialize the next filter in the chain, if any.
	return lzma_next_filter_init(&((struct lzma_coder_s*)next->coder)->next, allocator,
			filters + 1);
}
